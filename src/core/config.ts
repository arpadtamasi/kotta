...
export interface WorkspaceConfig {
  ...
  requireHumanSignApproval: boolean;
}

const DEFAULTS: WorkspaceConfig = { baseBranch: "main", protectedBranches: ["main", "master", "develop"], branchPattern: "{prefix}/{id}-{slug}", requireHumanSignApproval: false };
...
export function readWorkspaceConfig(root: string): WorkspaceConfig {
  ...
  const parsed = parse(readFileSync(path, "utf8")) as { workflow?: { require_human_sign_approval?: unknown }; git?: { base_branch?: unknown; protected_branches?: unknown; branch_pattern?: unknown } } | null;
  ...
  const requireHumanSignApproval = typeof parsed?.workflow?.require_human_sign_approval === "boolean"
    ? parsed.workflow.require_human_sign_approval
    : DEFAULTS.requireHumanSignApproval;
  return { baseBranch, protectedBranches: [...new Set([...configured, baseBranch])], branchPattern, requireHumanSignApproval };
}
