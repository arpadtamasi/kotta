...
One exact, entity-scoped transition is prepared: contract sign, close, cancel, or reopen; batch sign or close; observation resolve; or decision create.

## Main success scenario

The agent puts the decision to the operator in their language: what will happen, named by title, one line, then a plain yes or no - never an id, never a command to run. On an explicit yes in this conversation for this exact decision, the agent applies the validated mutation once, with approval recorded; the receipt links the visible human response.
