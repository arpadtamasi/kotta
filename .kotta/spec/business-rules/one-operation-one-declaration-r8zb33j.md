...
The CLI and the MCP tool surface. Totality is asserted by deriving both surfaces from the declaration and comparing sets — never by a count written into a specification, a task or a test, because a count is stale by the time it is read.
