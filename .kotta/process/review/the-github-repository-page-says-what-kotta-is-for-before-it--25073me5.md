...
coverage:
  ...
  Compatibility and migration reference material remains complete on the page and no longer precedes the product claim.:
    ...
    - EX-01m0hrxc4xrya8gfs08k289z5r
approved_by: cli
approved_at: '2026-08-21T11:51:58.831Z'
...
|---|---|
| A reader of the rendered repository page can state what Kotta is for, recognise whether they are the intended reader, and name something Kotta deliberately does not do, before the page presents an install command. | README.md opens with 'What Kotta is for' (line 11, three short paragraphs), 'Where it fits' (line 22), 'What you get' (line 50) and 'What Kotta is not' (line 63); the first install command is at line 74. 'Where it fits' names the reader in its first sentence and then gives the three situations they arrive from — new software, a legacy rewrite, and an existing codebase — each with a concrete example. 'What Kotta is not' states that Kotta is not an agent, is local and file-based, and has no hosted service, scheduler daemon or Jira/Linear sync. Claims checked against the workspace: the workshops named exist in skills/; the spec forms named (goal, example, state-machine, business-rule, glossary-term, use-case) exist in .kotta/spec/forms/; 'shaping runs without a task' is rule 1 of .kotta/AGENTS.md; the kotta gap behaviour is the one the CLI overview section already documents; 'one human gate, at close' is QA-01m0fp2hdkq55yrx9qr5t8pweh. |
