...
|---|---|
| In a repository with one checkout on `claude/harness-branch`, `kotta status`, `kotta contract new`, | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| `start` in that repository creates no branch and no worktree; the contract and the claim both name | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| `close` on that contract removes no worktree and deletes no branch, and says so. | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| `cancel` on that contract likewise leaves branch and checkout in place. | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| `start` in a single checkout on `main` refuses, naming the protected branch, while `status` and | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| With several worktrees and one on the base branch, every existing test passes unchanged. | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| With several worktrees and none on the base branch, the present error message is unchanged. | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| Setting `git.branch_pattern` to a different pattern changes the branch `start` creates in a | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| A `branch_pattern` that renders to an invalid ref name is refused at `start`, naming the pattern. | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| `kotta status` names the resolved control plane and whether it was a single checkout. | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| `kotta validate`, `npm run typecheck`, `npm run build` and the full suite pass. | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| workflow: happy_path_verified | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| workflow: failure_and_cancellation_paths_verified | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |
| workflow: authorization_and_idempotency_verified | Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.  resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.  startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.  readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.  schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.  Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI. |

### Verification performed

Implemented on feat/T-01kztvgdxb60hx200mbrgs9kmh-a-hosted-session-runs-kotta-on-the-one-branch-it-was-given at fa25a9b, branched from main.

resolveControlPlane (src/git/control-plane.ts) resolves by worktree count first: exactly one checkout is the control plane on whatever branch it holds, reported as mode "single" with that branch; the linked cases and both refusal messages are unchanged. controlPlaneRoot is now a thin wrapper, so every existing caller — including statusCommand, the read path that failed — goes through the same resolution.

startContract adopts when the control plane is "single" and the branch is not protected: it records that branch and "." as the worktree, creates neither, and writes origin: adopted on the claim and branch_origin on the contract. closeContract, cancelContract and reviewContract read that flag: review checks the adopted checkout for cleanliness instead of a .worktrees path that does not exist, and close and cancel skip worktree removal and branch deletion entirely, reporting what they preserved.

readWorkspaceConfig now reads git.branch_pattern, and branchName renders {prefix}, {id} and {slug} from it. The shipped default reproduces the previous names byte for byte — the whole existing suite, which asserts concrete branch names throughout, is the evidence. A pattern rendering to a name Git rejects is refused at start, naming the pattern and the value.

schemas/claim.schema.json gained origin and also execution_mode, which startContract has been writing for some time while the schema had additionalProperties: false and did not list it; nothing validated claims against the JSON schema, so it had gone unnoticed — validateClaim (src/core/claim.ts) is the path that runs, and it now accepts and constrains origin.

Verification: a new tests/integration/single-checkout.test.ts runs the reported reproduction as measured — init on main, commit, checkout -b claude/harness-branch — then the whole lifecycle through close, asserting mode "single", origin "adopted", exactly two branches, no .worktrees directory, the claim's recorded worktree ".", and that after close the checkout is still on claude/harness-branch with its delivered file. Five more cover cancel preserving an adopted branch, the ordinary single-checkout-on-main path still creating branch and worktree, the unchanged multi-worktree refusal, a custom branch_pattern changing the created name, and an invalid pattern being refused. npm run typecheck, npm run build and npx vitest run --exclude '.worktrees/**' all pass: 43 files, 288 passed, 1 skipped. Documentation: templates/AGENTS.md and README's control-plane paragraph; .kotta/AGENTS.md regenerated by the built CLI.

### Deviations

One, and the suite found it. The contract's acceptance said "start in a single checkout on main refuses, naming the protected branch, while status and contract new in the same checkout succeed". Implemented literally, that broke 20+ existing tests: the ordinary local repository is also a single checkout on main, and every fixture that signs a contract and starts it was refused. The rule was wrong, not the tests.

Adoption is therefore conditioned on the branch being non-protected. A single checkout on a protected branch is the ordinary local case, not a host-chosen work branch, so Kotta names and creates as it always has — which is also how execution stays off the protected branch, rather than by refusing to start at all. The decision's sentence that a single checkout on main "is a control plane and never an execution target" still holds exactly: state is written there, and the work happens on the new branch in the new worktree. What changes is only that the outcome is a normal start instead of an error. The test asserting the refusal was replaced by one asserting the created branch, the created worktree, and that HEAD is still main.

Nothing else in the acceptance changed. A detached-HEAD single checkout also falls back to creating rather than erroring, for the same reason: there is no branch to adopt.

### Observations created

None. Everything found was inside the contract's scope, including the claim-schema drift the contract already named in its execution notes.

### Known concerns

In adopted mode the lifecycle commits land on the host's branch, which is intended — that branch is the session's only place — but it means the contract's state changes and the implementation share a history. A reader of that branch sees chore(kotta) commits interleaved with the work. There is no divergence risk, because there is nothing to diverge from, but it is a different reading experience from the multi-worktree shape.

close in adopted mode still requires the branch to be merged into HEAD. In a single checkout HEAD is that branch, so the check passes trivially and the contract can be closed without the work going anywhere. That matches what "integrated" can mean when the host owns the push, and it is worth a second look if hosted sessions later gain a base branch to merge into.

The board was not exercised against a single-checkout repository. The contract's execution notes flagged this as something to record rather than widen into; nothing suggests it fails, since kotta ui reads through the same resolution, but it is untested here.
