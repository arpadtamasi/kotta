...
F-01kzd9sh03y7hwwbeen5cp4s0q (integrate supports only codex), plus three executor defects recorded before this run: the implemented state never checks for change, the agent stdout is discarded, and --resume --agent does not update the claim. A fourth, F-...1me3jkh4, records that AGENT_ARGUMENTS.claude lacks a permission flag.

### Known concerns

Kotta has no execution event for the run that produced this work. The only recorded event is a no-op run reported as implemented; the resume that would have recorded the real run aborted on a dirty control worktree. The code is verified directly rather than through the lifecycle log.
