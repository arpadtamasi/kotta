...
2. Inspect every member state. Only unclaimed `defined` contracts are dispatch candidates; `review`
...
4. Ensure the configured base branch is checked out in the control worktree, then run
   ...
   configured base branch. Every newly dispatched contract starts from the coordinator's exact
...
8. For each active contract, the fresh agent follows the `execute-contract` contract. Poll with
