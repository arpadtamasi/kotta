...
The agent binary is resolved from `--agent` (`claude`, `codex`, or any command on `PATH`); `KOTTA_AGENT_COMMAND` overrides the executable, which is how the test suite drives a deterministic script double instead of a real agent. A supported agent is invoked so that it can actually change files — `claude` runs as `claude -p --permission-mode bypassPermissions` inside its own worktree — and an invocation that structurally cannot write fails at launch naming the cause instead of producing an empty run. Review, merge and close stay separate human gates — `execute` never enters them.
